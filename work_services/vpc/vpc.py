...
def list_vpc(file_path,session,region):
    client = session.client('ec2',region_name=region)
    sts = session.client('sts')
    account_id = sts.get_caller_identity()["Account"]
    client_list_object = client.describe_vpcs()
    client_list = []
    if len(client_list_object['Vpcs']) != 0:
        for i in client_list_object['Vpcs']:
            arn = f"arn:aws:ec2:{region}:{account_id}:vpc/{i['VpcId']}"
