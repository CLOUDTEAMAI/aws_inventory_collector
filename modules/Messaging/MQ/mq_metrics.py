...
def mq_metrics(file_path, session, region, account, metrics, time_generated, boto_config):
    next_token = None
    idx = 0
    client = session.client('mq', region_name=region, config=boto_config)
    ...
    while True:
        try:
            ...
            for resource in response.get('BrokerSummaries', []):
                inventory.append(resource['BrokerName'])
            if inventory:
