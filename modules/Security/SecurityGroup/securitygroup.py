...
def list_securitygroup(file_path, session, region, time_generated, account, boto_config):
    next_token = None
    idx = 0
    ...
    while True:
        try:
            ...
            for resource in response.get('SecurityGroups', []):
                arn = f"arn:aws:ec2:{region}:{account_id}:security-group/{resource['GroupId']}"
