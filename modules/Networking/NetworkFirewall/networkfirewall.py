...
def list_networkfirewall(file_path, session, region, time_generated, account, boto_config):
    next_token = None
    idx = 0
    ...
    while True:
        try:
            ...
            for i in response.get('Firewalls', []):
                arn = i['FirewallArn']
                firewall_description = client.describe_firewall(
                    FirewallArn=arn)
                inventory_object = extract_common_info(
                    arn, firewall_description.get('Firewall', {}), region, account_id, time_generated, account_name)
