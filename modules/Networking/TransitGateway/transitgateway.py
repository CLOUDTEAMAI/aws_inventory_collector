...
def list_transitgateway(file_path, session, region, time_generated, account):
    next_token = None
    idx = 0
...
def list_transitgateway_attachments(file_path, session, region, time_generated, account):
    next_token = None
    idx = 0
    client = session.client('ec2', region_name=region)
    account_id = account['account_id']
