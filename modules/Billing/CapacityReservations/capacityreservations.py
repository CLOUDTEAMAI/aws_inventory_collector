...
def list_capacityreservations(file_path, session, region, time_generated, account, boto_config):
    """
    ...
    while True:
        try:
            ...
            response = client.describe_capacity_reservations(
                NextToken=next_token) if next_token else client.describe_capacity_reservations()
            for resource in response.get('CapacityReservations', []):
                arn = resource.get('CapacityReservationArn', '')
                if 'StartDate' in resource:
                    resource['StartDate'] = resource['StartDate'].isoformat()
                if 'EndDate' in resource:
                    resource['EndDate'] = resource['EndDate'].isoformat()
                if 'CreateDate' in resource:
                    resource['CreateDate'] = resource['CreateDate'].isoformat()
